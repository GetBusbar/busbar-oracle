...
mint_raw="$(curl -sS -m 10 -w '\n%{http_code}' -X POST "http://127.0.0.1:${AP}/api/v1/admin/keys" -H "Authorization: Bearer $ADMIN" -H 'Content-Type: application/json' -d '{"name":"teller-meter","group":"oracle"}')"
...
if ! result="$(jq -n \
  ...
  '{mint_status:$mint_status, meter_status:$meter_status,
    ...
    usage_spend_cents_delta:$usage_spend_cents_delta}' 2>"$W/result.err")"; then
