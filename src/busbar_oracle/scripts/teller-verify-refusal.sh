...
# THE CODE IS READ, NOT ASSERTED. `-w` appends the real status as a last line, so `mint_status`
# below is what this binary answered rather than what the harness assumed; a mint that stopped being
# a 201 is then a diff on this cell.
mint_raw="$(curl -sS -m 10 -w '\n%{http_code}' -X POST "http://127.0.0.1:${AP}/api/v1/admin/keys" -H "Authorization: Bearer $ADMIN" -H 'Content-Type: application/json' \
...
if ! result="$(jq -n \
  ...
  '{mint_status:$mint_status, verify_status:$verify_status, verify_body:$verify_body,
    egress_count:$egress_count, usage_requests_delta:$usage_requests_delta}' 2>"$W/result.err")"; then
